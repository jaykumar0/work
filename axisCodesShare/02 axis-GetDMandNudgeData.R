...
###
# Process nudge and all data Day wise
###

# Get Frequencies

md <- dcast(m,EmpID + NudgeDt ~ msg_cat5, value.var = "Freq",fun.aggregate = sum)

names(md)
md$TotNudge <- rowSums(md[c(3:7,9)])
#md1$TotNudge <- md1$CASA + md1$CI + md1$Engaged + md1$Generic + md1$LIMeeting + md1$NotEngaged

md$Specific <- md$CASA + md$CI + md$Engaged + md$LIMeeting + md$NotEngaged
...
# mark NA to 0 , all are CASAAchPercent
t1[is.na(t1)] <- 0
